...
use antikythera_core::application::tooling::interface::{PROTOCOL_VERSION, ServerToolInfo};
...
impl McpTransport for McpProcess {
    ...
    async fn tool_metadata(&self, tool: &str) -> Option<ServerToolInfo> {
        self.tool_metadata(tool).await
    }

    async fn list_tools(&self) -> Vec<ServerToolInfo> {
        self.inner.tool_cache.lock().await.values().cloned().collect()
    }
