...
use antikythera_core::application::client::{ChatRequest, ChatResult, McpClient};
use antikythera_core::application::model_provider::ModelProvider;
use antikythera_core::application::config::AppConfig;
...
async fn handle_command<P: ModelProvider>(
    ...
    match name.as_str() {
        ...
        "reload" => {
            ...
            match AppConfig::load(Some(Path::new(antikythera_core::application::config::CONFIG_PATH))) {
                Ok(config) => {
        ...
        "config" => {
            ...
            match action.as_deref() {
                Some("edit") => {
                    #[cfg(feature = "wizard")]
                    {
                        match antikythera_core::config::wizard::run_setup_menu().await {
                            Ok(_) => {
