...
The **Runtime Bridge** is the connectivity layer that lets the WASM core run on either side (browser or server) while resources — LLM, tools, hooks — stay reachable from both. It ships two host runtimes plus a normative wire protocol:
